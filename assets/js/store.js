...
function productCard(p){
  const dots=p.colors.map(c=>`<span class="dot" title="${cname(c.name)}" style="background:${c.hex}"></span>`).join('');
  return `<div class="card">
    <a class="card-media" href="product.html?id=${p.id}">
      ${p.badge?`<span class="badge">${badgeTx(p.badge)}</span>`:''}
      <img loading="lazy" src="${p.colors[0].img}" alt="${pname(p)}">
    </a>
    <div class="card-body">
      <a href="product.html?id=${p.id}"><h3 class="card-title">${pname(p)}</h3></a>
      <div>${stars(p.rating)} <small style="color:#6b7a74">(${num(p.reviews)} ${t('card_reviews')})</small></div>
      ...
      <div style="display:flex;gap:.5rem;margin-top:auto">
        <a class="btn btn-green" style="flex:1;justify-content:center;padding:.65rem" href="product.html?id=${p.id}">${t('card_details')}</a>
        <button class="btn btn-gold" style="padding:.65rem 1rem" onclick="addToCart('${p.id}','${p.colors[0].name}',1)">${t('card_add')}</button>
      </div>
    </div>
  </div>`;
}
